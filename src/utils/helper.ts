...
export function checkFileType(
  file: FileObject | any
): 'Image' | 'Excel' | 'CSV' | 'URL' | 'Unknown type' {
  const mimeType = file?.type ? file.type : 'url';
...
export const uploadImage = async (files: any) => {
  const data = { id: '', path: '', error: '' };
  try {
    const formData = new FormData();
    formData.append('image', files);
    const res: any = await axios.post('/v1/partner/marketplace/upload', formData, {
      headers: {
